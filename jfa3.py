...
def jump_flood_2D(grid, max_iter=100, device=device_cpu):
    height, width = grid.shape
    jump = max(height, width) // 2

    for step in range(max_iter):
        # For each jump, we apply a max pooling operation over a kernel
        # of 3x3 (covering 8 neighbors)
        grid_min = F.max_pool2d(-grid.unsqueeze(0).unsqueeze(0), 3, stride=1, padding=1)
        grid_min = -grid_min.squeeze(0).squeeze(0)
        # Adaptive jump update based on current step for smoother transitions
        adaptive_jump = jump #/ (step + 1) ** 0.5
        grid = torch.min(grid, grid_min + adaptive_jump)
        # Reduce the jump by half
        jump = max(jump // 2, 1)
...
def jump_flood_3D(grid, max_iter=5, device=device_cpu):
    ...
    for step in range(max_iter):
        # Apply max pooling over a 3x3x3 kernel to get neighbors
        grid_min = F.max_pool3d(-grid.unsqueeze(0).unsqueeze(0), 3, stride=1, padding=1)
        grid_min = -grid_min.squeeze(0).squeeze(0)
        # Adaptive jump update based on current step for smoother transitions
        adaptive_jump = jump #/ (step + 1) ** 0.5
        grid = torch.min(grid, grid_min + adaptive_jump)
...
def benchmark_jfa_2d_with_cucim(shape, seed_positions, max_iter=1000):
    # Initialize seed grids for CPU and GPU
    grid_cpu = initialize_seed_grid(shape, seed_positions, device_cpu)
    grid_gpu = initialize_seed_grid(shape, seed_positions, device_gpu)
