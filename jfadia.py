...
def jump_flood_3D(grid, max_iter=10, device='cpu'):
    depth, height, width = grid.shape
    z, y, x = torch.meshgrid(torch.arange(depth, device=device), torch.arange(height, device=device), torch.arange(width, device=device))
    coords = torch.stack((z, y, x), dim=-1)
    jump = max(depth, height, width) // 2
    
    # Initial seeds: cells with zero values are treated as initial seeds
    seeds = grid == 0
    
    for step in range(max_iter):
        # Set of new seeds for the next iteration
        new_seeds = torch.zeros_like(seeds, dtype=torch.bool)

        # Loop through each neighboring direction based on the current jump distance
        ...
        for n in neighbors:
            # Clamp neighbor coordinates to stay within bounds of the grid
            n[..., 0].clamp_(0, depth - 1)
            n[..., 1].clamp_(0, height - 1)
            n[..., 2].clamp_(0, width - 1)
            
            # Calculate the distance from the current cell to the neighbor
            distance = torch.sqrt(((coords - n) ** 2).sum(dim=-1))
            new_values = grid[n[..., 0], n[..., 1], n[..., 2]] + distance
            
            # Check which cells should be updated
            updated_cells = new_values < grid
            grid = torch.min(grid, new_values)
            
            # Mark updated cells as new seeds for the next iteration
            new_seeds |= updated_cells

        # Update the set of seeds for the next iteration
        seeds = new_seeds
        
        # Halve the jump distance, ensuring it stays at least 1
        jump = max(jump // 2, 1)
    
    return grid

# Example usage (3D)
grid = torch.full((5, 10, 10), float('inf'))
grid[0, 0, 0] = 0  # Set the initial seed point at the top-left-front corner
updated_grid = jump_flood_3D(grid, max_iter=10)
print(updated_grid)


def jump_flood_2D(grid, max_iter=10, device='cpu'):
    ...
    for step in range(max_iter):
        ...
        for n in neighbors:
            ...
            # Calculate the distance from the current cell to the neighbor
            distance = torch.sqrt(((coords - n) ** 2).sum(dim=-1))
            new_values = grid[n[..., 0], n[..., 1]] + distance
...
# Example usage
grid = torch.full((10, 10, 10), float('inf'))
grid[0, 0, 0] = 0  # Set the initial seed point at the top-left-front corner
updated_grid = jump_flood_3D(grid, max_iter=10)
print(updated_grid)
...
def benchmark_jfa_with_cucim(shape, seed_positions, max_iter=5, dim=2):
...
complex_cases = [
    ...
    {"shape": (128, 128, 128), "seeds": 20, "max_iter": 100},  # Non-cubic case
]
